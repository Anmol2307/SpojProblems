...
int main() {
	...
	while (t--) {
		...
		for (int i=0; i < m; i++) {
	 		...
	 		long res=0;
	 		if (x2 > y1) {
	 			res = query_tree(1,0,n-1,x1,y1).right;
	 			// cout << "1 " << endl;
	 			res+= query_tree(1,0,n-1,y1+1,x2-1).sum;
	 			// cout << "2" << endl;
	 			res+= query_tree(1,0,n-1,x2,y2).left;
	 			// cout << "3" << endl;
	 		}
	 		else {
	 			res = query_tree(1,0,n-1,x2,y1).best;
	 			res = max(res,max(query_tree(1,0,n-1,x1,y1).right + query_tree(1,0,n-1,y1+1,y2).left,query_tree(1,0,n-1,x1,x2-1).right + query_tree(1,0,n-1,x2,y2).left));
